...
```
tools/
└── gen_vocab_pdf.py                      ← Python (fpdf2); parses Dart data files,
                                             outputs vocabulary.pdf (A4, per-level tables)
...
| Job            | Needs   | Runner          | Output                                    |
|----------------|---------|-----------------|-------------------------------------------|
| `analyze`      | —       | ubuntu-latest   | lint + unit tests (gate for build jobs)   |
| `build-android`| analyze | ubuntu-latest   | `LanguageTrainer-*-android.apk` artifact  |
| `build-linux`  | analyze | ubuntu-latest   | `LanguageTrainer-*-linux-amd64.deb`       |
| `build-macos`  | analyze | macos-latest    | `LanguageTrainer-*-macos.zip`             |
| `vocab-pdf`    | —       | ubuntu-latest   | `vocabulary.pdf` artifact (90-day retain) |
| `sbom`         | analyze | ubuntu-latest   | CycloneDX SBOM JSON                       |

`vocab-pdf` is independent of the Flutter SDK; it runs `tools/gen_vocab_pdf.py`
with `fpdf2` and needs only Python 3.12.
