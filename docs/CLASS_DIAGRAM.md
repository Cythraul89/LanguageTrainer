...
class AdjReverseQuizItem {
  +AdjectiveEntry entry
}
class VerbSeparableQuizItem {
  +String infinitive
  +String english
  +String prefix
}
class PrepTranslationQuizItem {
  +PrepositionEntry entry
}
class PrepCaseQuizItem {
  +PrepositionEntry entry
}
...
QuizItem <|-- AdjReverseQuizItem
QuizItem <|-- VerbSeparableQuizItem
QuizItem <|-- PrepTranslationQuizItem
QuizItem <|-- PrepCaseQuizItem
...
AdjReverseQuizItem --> AdjectiveEntry
PrepTranslationQuizItem --> PrepositionEntry
PrepCaseQuizItem --> PrepositionEntry
NounEntry --> Article
NounEntry --> CefrLevel
VerbEntry --> Auxiliary
VerbEntry --> CefrLevel
...
- `QuizItem` is a Dart **sealed class** with 16 subtypes; pattern matching in `QuizScreen` is exhaustive.
